...
void SensorControllerTask(void *params)
{
	do {
		...
		inisialize_hostPC_datalinik();

		if (parse_hostPC_message() = "PC_Command_START") {

		} else if (parse_hostPC_message() = "PC_Command_RESET") {

		} else if (parse_hostPC_message() = "PC_Command_NONE") {
